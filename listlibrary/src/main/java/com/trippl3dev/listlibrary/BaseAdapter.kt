...
class BaseAdapter<T>(holderInterface: IAdaptee<T>?, context: Context?) :
    RecyclerView.Adapter<GenericHolder>() {
    ...
    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): GenericHolder {
        return if (viewType == adaptee?.getErrorLoadingType(state)) {
            ...
            adaptee?.onGreateHolder(v, parent)
            GenericHolder(v)
        }
    }
    ...
    override fun getItemCount(): Int {
        ...
        if (state == 0) {
            ...
            if (adaptee?.getIsCircular()!!) Int.MAX_VALUE else
                size = getCurrentGenerictList()?.size!! + 1 ?: 0
        }
//        if(size == 0 && !iEmpty.isEmpty()!!)
//            iEmpty.setEmpty(getCurrentGenerictList()?.size == 0)
//        else if(size > 0 && iEmpty.isEmpty()!!)
//            iEmpty.setEmpty(getCurrentGenerictList()?.size == 0)
        return size
    }
    ...
    override fun onBindViewHolder(holder: GenericHolder, position: Int) {
        if (holder.itemViewType == adaptee?.getErrorLoadingType(state)) {
            ...
            try {
                binding?.setVariable(
                    BR.model,
                    if (adaptee?.getIsCircular()!!) getCurrentGenerictList()?.get(position % getCurrentGenerictList()?.size!!) else getCurrentGenerictList()?.get(
                        position
                    )
                )
